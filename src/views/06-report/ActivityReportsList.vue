        ...
        <div class="container">
            ...
            <div v-else class="ts-box">
                ...
                <tbody>
                    ...
                    <td>
                        <a :href="getActivityLink(report.reportItemId)" target="_blank">
                    ...
                    <td colspan="5" class="p-0">
                        ...
                        <div class="card card-body">
                            <!-- <p><strong>活動ID:</strong> {{ report.reportItemId }}</p> -->
                            <p><strong>活動ID:</strong> 
                                <a :href="getActivityLink(report.reportItemId)" target="_blank">
                                    {{ report.reportItemId }}
                                </a>
                            </p>
...
async function fetchActivityReports() {
    try {
        loading.value = true;
        error.value = null;
        const response = await axiosapi.get('/report/list/activities');
        reports.value = response.data;
        console.log("Fetched activity reports:", reports.value);
    } catch (err) {
        console.error("獲取活動檢舉列表失敗:", err);
        error.value = "獲取活動檢舉列表失敗，請稍後再試。";
    } finally {
        loading.value = false;
    }
}
