        ...
        <div class="row bg-light rounded mx-0">
            ...
            <div v-else class="ts-box">
                ...
                <tbody>
                    ...
                    <td>
                        <a :href="getActivityLink(report.associatedActivityId)" target="_blank">
                            {{ truncateContent(report.reportedContent) }}
                        </a>
                    </td>
                    ...
                    <td colspan="5" class="p-0">
                        ...
                        <div class="card card-body">
                            <p><strong>留言所屬活動ID:</strong> 
                            <a :href="getActivityLink(report.associatedActivityId)" target="_blank">
                                {{ report.associatedActivityId }}
                            </a>
                            </p>
                            <p><strong>留言者:</strong> {{ report.commentUserName }}</p>
...
async function fetchCommentReports() {
    try {
        loading.value = true;
        error.value = null;
        const response = await axiosapi.get('/report/list/actComments');
        reports.value = response.data;
        console.log("Fetched comment reports:", reports.value);
    } catch (err) {
        console.error("獲取評論檢舉列表失敗:", err);
        error.value = "獲取評論檢舉列表失敗，請稍後再試。";
    } finally {
        loading.value = false;
    }
}
...
function getActivityLink(activityId) {
    return `${MEETVUE_BASE_URL}/briefactivities/${activityId}`;
}
